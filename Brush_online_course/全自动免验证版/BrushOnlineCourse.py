...
class BrushOnlineCourse:
    ...
    def login(self):
        ...
        scrrenshot_path = './screenshot/veryCode.png'

        while self.browser.find_element_by_xpath('//*[@id="errorMsg"]/span').text == '':
            self.browser.find_element_by_xpath('//*[@id="changeImage"]/img').screenshot(scrrenshot_path)  # 下载验证码
            ocr = Demo_ocr()
            verif_code = ocr.OCR_img(scrrenshot_path)
            print('验证码为:' + verif_code)
            self.browser.find_element_by_id('loginName').send_keys(self.account)
            self.browser.find_element_by_id('password').send_keys(self.password)
            self.browser.find_element_by_id('checkCode').send_keys(verif_code)
            print('输入完毕')
            self.browser.find_element_by_xpath('//*[@id="userLogin"]').click()
            ...
            if self.browser.current_url == self.url_login:
                ...
                print(self.browser.find_element_by_xpath('//*[@id="errorMsg"]/span').text)
    ...
    def verifCode(self):
        self.browser.find_element_by_xpath('//*[@id="changeImage"]/img').screenshot('./screenshot/veryCode.png')
        pass
    ...
    def checkVideoIsEnd(self):
        ...
        while self.browser.find_element_by_xpath(
            ...
            print(self.browser.find_element_by_xpath(
                '//*[@id="mediaplayer_controlbar"]/span[4]/span[1]/span[2]/span[3]').get_attribute('style'))
        print('恭喜刷完本章:' + self.browser.find_element_by_xpath(
            '/html/body/div[3]/div[2]/div[2]/div/div[2]/div/div/div[1]/div/ul/li/a').get_attribute('itemname'))
        return True
