...
class View
{
    ...
    public static function addAdminSidebarRouter(Router $router)
    {
    ...
    private static function addSidebarConfig(string $module)
    {
        ...
        if (is_array($sidebar)) {
            self::$adminsidebar=array_merge(self::$adminsidebar, $sidebar);
        }
    }
            ...
            if ($parent) {
                ...
                if (isset($adminsidebar['child']) && is_array($adminsidebar['child'])) {
                    foreach ($adminsidebar['child'] as $name=>$child) {
                        ...
                        $routerName = Router::getInstance()->getRouterFullName($name);
                        $routerMapping = Router::getInstance()->getRouter($routerName);
                        if ($routerMapping) {
                            $cid=count($sidebar[$id]['child']??[]);
                            self::$childsidebar[$parent][]= $routerName;
                            ...
                            $routerMapping->setParam([
                                'config'=> $sidebar[$id]['child'][$cid],
                                'isChild'=>true,
                                'parent'=>$sidebar[$id],
                            ]);
                            Router::getInstance()->refreshMapping($routerMapping);
                        }
                    }
                }
            }
    ...
    private static function transAdminSidebarTree(array $input)
    {
        ...
        foreach ($input as $name=>$adminsidebar) {
            ...
            // 父节点没有子节点
            if (!isset($sidebar[$id]['child'])) {
                // 验证权限
                if (isset($adminsidebar['acl']) && !static::checkMe($adminsidebar['acl'])) {
                    unset($sidebar[$id]);
                }
            }
        }
    ...
    {
        return Context::getInstance()->getVisitor()->hasPermission(new Permission($permission));
    }
}
